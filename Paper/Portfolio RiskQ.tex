...
We validated probability normalization and tail-mask correctness, then compared IQAE estimates to true tail probability on toy data using the same $\ell$ and histogram for classical and quantum paths. Results show classical Monte Carlo error decreasing with sample size (consistent with $1/\sqrt{N}$ behavior), while IQAE achieves low tail-probability error with a small number of Grover iterations, as illustrated in Fig.~\ref{fig:exp-classical-quantum}. Discretization and noise sweeps identify a stable operating regime where comparisons remain reliable (Fig.~\ref{fig:exp-discretization-noise}). We also tracked resource usage (qubits, depth, 2Q gates, oracle calls) and summarized feasibility via the boundary analysis in Fig.~\ref{fig:feasibility-boundary}. The dashboard reports VaR/CVaR and a tail-highlighted histogram alongside IQAE confidence intervals and absolute/relative error (Fig.~\ref{fig:dashboard}).
...
Left panel collects portfolio inputs and stress settings; right panel reports VaR/CVaR, a tail-highlighted histogram, and a classical vs quantum comparison (Fig.~\ref{fig:dashboard}). IQAE reports the same tail probability with a confidence interval and absolute/relative error. The control panel includes portfolio weights and horizon days, a return model selector (normal vs crash-mixture), an optional tail threshold override, and a backend toggle between classical and IQAE. Stress sliders adjust volatility multiplier, correlation spike, and mean shock, while crash-mixture sliders set crash probability, crash mean shift, and crash volatility jump. When IQAE is enabled, the histogram qubit slider sets the number of loss bins used for discretization.
\begin{figure}[!t]
    \centering
